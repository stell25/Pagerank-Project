...
class WebGraph():
    ...
    def __init__(self, filename, max_nnz=None, filter_ratio=None):
        ...
        with gzip.open(filename,newline='',mode='rt') as f:
            for i,row in enumerate(csv.DictReader(f)):
                ...
                target_counts[target] += 1 #in-link number
        ...
        n = len(self.url_dict)
        self.P = torch.sparse_coo_tensor(i, v, torch.Size([n,n])) #P matrix
    ...
    def power_method(self, v=None, x0=None, alpha=0.85, max_iterations=1000, epsilon=1e-6):
        ...
        '''
        with torch.no_grad():
            n = self.P.shape[0] #dimension of P
            ...
            row_sum = torch.sparse.sum(self.P, 1).indices()
            a = torch.ones([n,1])
            a[row_sum] = 0
            ...
            for i in range(max_iterations):
                ...
                mat1 = (alpha * x.t() @ a + (1 - alpha)) * v.t()
                x = torch.sparse.addmm(mat1.t(), self.P.t(), x, beta=1, alpha = alpha)
                x /= torch.norm(x)
                # compute the new x vector using Eq (5.1)
                # FIXME: Task 1
                # HINT: this can be done with a single call to the `torch.sparse.addmm` function,
                #torch.sparse.addmm(mat1, mat2, mat3, *, beta=1, alpha=1, out=None)
